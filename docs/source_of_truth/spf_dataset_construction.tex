...
x_{18,t} &\equiv \text{CPI10-Q2}_t
 - \frac{1}{40}\text{CPI1-Q2}_t, \\
x_{19,t} &\equiv \text{CPI10-Q3}_t
 - \frac{1}{40}\left(\text{CPI1-Q2}_t + \text{CPI1-Q3}_t\right), \\
x_{20,t} &\equiv \text{CPI10-Q4}_t
 - \frac{1}{40}\left(\text{CPI1-Q2}_t + \text{CPI1-Q3}_t
 + \text{CPI1-Q4}_t\right).
\end{align}
The corresponding forward-looking measurement equations are
\begin{align}
x_{17,t} &= \frac{1}{40}\sum_{j=0}^{39} \pi_{t-1+j \to t+j}
 + \varepsilon_{17,t}, \\
x_{18,t} &= \frac{1}{40}\sum_{j=0}^{38} \pi_{t-1+j \to t+j}
 + \varepsilon_{18,t}, \\
x_{19,t} &= \frac{1}{40}\sum_{j=0}^{37} \pi_{t-1+j \to t+j}
 + \varepsilon_{19,t}, \\
x_{20,t} &= \frac{1}{40}\sum_{j=0}^{36} \pi_{t-1+j \to t+j}
 + \varepsilon_{20,t}.
\end{align}
Under this quarterly representation, CPI1 enters the raw 10-year forecast with
weight $1/40$ once it is realized. In Q2, the correction is exactly
$\text{CPI1}/40$; in Q3 and Q4, CPI1 remains one term in a larger realized
block. Thus the raw CPI10 measure mechanically contains more realized inflation
in Q4 than in Q1.
...
\For{each row key $(\textit{survey\_year}, \textit{survey\_quarter}, \textit{forecaster\_id})$}
    ...
    \ElsIf{$\textit{survey\_quarter} = 2$}
        ...
        \State $\textit{adjusted} \gets \textit{cpi10} - \textit{cpi1\_q2}/40$
    \ElsIf{$\textit{survey\_quarter} = 3$}
        ...
        \State $\textit{adjusted} \gets \textit{cpi10} - (\textit{cpi1\_q2} + \textit{cpi1\_q3})/40$
    \Else
        \State $\textit{key\_q2} \gets (\textit{forecast\_individual}, \textit{survey\_year}, 2, \textit{forecaster\_id})$
        \State $\textit{key\_q3} \gets (\textit{forecast\_individual}, \textit{survey\_year}, 3, \textit{forecaster\_id})$
        ...
        \State $\textit{adjusted} \gets \textit{cpi10} - (\textit{cpi1\_q2} + \textit{cpi1\_q3} + \textit{cpi1\_q4})/40$
    \EndIf
