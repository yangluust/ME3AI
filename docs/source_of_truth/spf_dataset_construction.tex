...
$\text{CPI1}/40$; in Q3 and Q4, CPI1 remains one term in a larger realized block.
Thus the raw CPI10 measure mechanically contains more realized inflation in Q4
than in Q1.
...
\State Filter to rows with $\textit{variable}=\texttt{CPI}$.
...
\For{each row key $(\textit{survey\_year}, \textit{survey\_quarter}, \textit{forecaster\_id})$}
    \Statex \quad in rows with $\textit{variable}=\texttt{CPI}$
