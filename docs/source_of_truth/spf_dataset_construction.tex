...
For the construction of reputation and the regression dataset, let $x_{u,s}$
denote the raw SPF 10-year CPI forecast,
\begin{equation}
x_{u,s} \equiv \text{CPI10}_{u,s},
\end{equation}
where $u$ is \texttt{forecaster\_id}. This construction intentionally uses raw
\texttt{CPI10} rather than \texttt{adjusted\_cpi10}. Define the reputation
measure $\rho$ from
\begin{equation}
...
values and the raw \texttt{CPI10} forecast dated at the same survey $s$. The
constructed output is written as
\texttt{reputation\_measure.csv} with columns
\texttt{(survey\_year, survey\_quarter, forecaster\_id, rho)}.

\subsection{Construct regression dataset}

The regression dataset is indexed at the survey-date level. For each survey
date $s$, define the individual raw 10-year CPI forecast revision for unit $u$
by
\begin{equation}
...
The three OLS regressions are estimated at the survey level with a constant:
\begin{align}
...
\end{align}
The estimation sample is restricted to survey dates from 1991:Q4 through
2024:Q2, inclusive.
...
forecast from 1991:Q4 through 2024:Q2. The figure should plot the cumulative
sum of the survey-level data series $\bar{r}_s$ against the cumulative sums of
the fitted counterparts produced by the three regression models, using the same
bounded survey window in each series. Plot the data using a black dashed line.
Plot the fitted counterpart from Model 1 using a magenta solid line, Model 2
using a blue solid line, and Model 3 using a red solid line.

\section{Regression results}

The regression implementation writes a results table and a comparison figure to
\texttt{output/forecast\_revision/}. The table is
\texttt{regression\_results.csv}; it contains one row for each model and the
reported statistics described above: coefficient estimate, p value, adjusted
$R^2$, RMSE, and sample size. The figure is
\texttt{cumulative\_forecast\_revision\_comparison.png}; it plots the cumulative
change in long-term inflation forecast from 1991:Q4 through 2024:Q2 for the
data series and
for the fitted counterparts from Models 1, 2, and 3 using the specified line
styles and colors.
...
\begin{algorithmic}[1]
\State \textbf{Require} $\textit{x\_table}$: table containing raw \texttt{CPI10} as \texttt{x}.
...
Obtaining official Excel files may be implemented by \textsc{DownloadByVariableNames}: given variable names (e.g., CPI10, NGDP), the procedure uses \textsc{VariablePageURL} to form each variable's data page URL (e.g., \url{https://www.philadelphiafed.org/surveys-and-data/cpi10}), \textsc{GetDownloadLinks} to parse the page for Individual (and optionally other) links, and \textsc{DownloadOneFile} to save each file to \texttt{data-pipeline/input/} under the same filename as on the site. The currently implemented cleaned individual-level outputs are \texttt{forecast\_individual}, \texttt{forecaster\_survey}, \texttt{adjusted\_cpi10}, \texttt{inflation\_news}, \texttt{reputation\_measure}, and \texttt{regression\_dataset}, all written as CSV. The table \texttt{adjusted\_cpi10} remains available as a separate adjusted long-term expectation object, but the constructions of \texttt{reputation\_measure} and \texttt{regression\_dataset} use raw \texttt{CPI10} from \texttt{forecast\_individual}. Under the forecast revision regression design, the analysis results are written to \texttt{output/forecast\_revision/}, including \texttt{regression\_results.csv} and \texttt{cumulative\_forecast\_revision\_comparison.png}. Level/mean/median and probability tables remain out of scope unless added explicitly later.
