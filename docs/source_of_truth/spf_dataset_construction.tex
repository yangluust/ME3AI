...
For each forecaster and survey quarter, inflation news is defined as the
...
current survey quarter, while the nowcast term is \texttt{CPI2} from the
previous survey quarter for the same \texttt{forecaster\_id}. Thus, for
forecaster $i$ in quarter $(y,q)$,
\begin{align}
\text{news}_{i,y,q} &\equiv \text{CPI1}_{i,y,q} - \text{CPI2}_{i,y,q-1},
&& q \in \{2,3,4\}, \\
\text{news}_{i,y,1} &\equiv \text{CPI1}_{i,y,1} - \text{CPI2}_{i,y-1,4}.
\end{align}
Equivalently, inflation news at $(y,q)$ is formed by matching the current row
$(y,q,i)$ to the lagged survey row $(y,q-1,i)$ when $q \in \{2,3,4\}$, and to
$(y-1,4,i)$ when $q=1$. This construction respects SPF timing: \texttt{CPI2} in
survey $(y,q-1)$ is the nowcast for inflation realized in quarter $(y,q)$, so
the difference isolates the surprise in the realized current-quarter inflation
relative to the forecast available one survey earlier.

\subsection{Construct reputation measure}

Let $x$ denote the long-term inflation expectation used in the forecast
...
\begin{equation}
x = \rho\left((1-q)\pi_{\text{target}} + q z_a\right)
 + (1-\rho)\left((1-q)\pi_{\text{NE}} + q z_{\alpha}\right).
\end{equation}
Given values for $q$, $\pi_{\text{target}}$, $\pi_{\text{NE}}$, $z_a$, and
$z_{\alpha}$, this equation implies
\begin{equation}
\rho =
\frac{x - \left((1-q)\pi_{\text{NE}} + q z_{\alpha}\right)}
...
$x$ is available, compute $\rho$ using the configured parameter values and the
constructed long-term inflation expectation.
