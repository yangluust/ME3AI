...
a survey date $s=(y,q)$. The unit index $u$ may denote an individual
forecaster, or an aggregate forecast object such as a mean or median series.
...
$x_{u,s}$ used in the forecast revision regression. For individual forecasts,
$u$ corresponds to \texttt{forecaster\_id}; for later mean or median
constructions, $u$ can instead denote the corresponding aggregate unit.
...
revision regression. The construction of $x$ is intentionally flexible: it may
be an adjusted individual CPI10 forecast, or an adjusted mean or median CPI10
forecast, provided it is organized as a table indexed by survey date and any
optional forecast-identity key. Define the reputation measure $\rho$ from
\begin{equation}
...
\Statex \quad $(\textit{survey\_year}, \textit{survey\_quarter}, \textit{x})$ and may also contain
\Statex \quad an optional forecast-identity key such as \texttt{forecaster\_id} or \texttt{summary\_type}.
