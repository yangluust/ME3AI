...
expectation $x_{u,s}$ is defined for the forecast revision pipeline. If the
config selects raw \texttt{CPI10}, then
\begin{equation}
x_{u,s} \equiv \text{CPI10}_{u,s}.
\end{equation}
If instead the config selects adjusted \texttt{CPI10}, then $x_{u,s}$ equals
the adjusted long-term inflation expectation constructed in the previous
subsection and stored in \texttt{adjusted\_cpi10.csv}. In either case, $u$
corresponds to \texttt{forecaster\_id}. The same config-selected definition of
$x_{u,s}$ is used in both \texttt{reputation\_measure.csv} and
...
choice of long-term inflation expectation from the dedicated regression config
file. For each observation where $x_{u,s}$ is available, compute $\rho_{u,s}$
using the configured parameter values and the config-selected definition of
$x_{u,s}$ dated at the same survey $s$. The constructed output is written as
...
estimation and fitted-value comparisons.
...
the three regression models, using the same bounded survey window in each
...
\begin{algorithmic}[1]
\State \textbf{Require} $\textit{regression\_config}$ contains \texttt{x\_definition}.
...
\State Keep only survey dates inside the configured sample window.
